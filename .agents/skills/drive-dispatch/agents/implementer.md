...
(a) you have explicit parallel work to do during the wait _and_ (b) the
...
- Prose attributions like "out of scope per spec § 4", "deferred to T5.6",
  "per CKPT-2 decision" → the _constraint_ belongs in the comment, not the
...
ticket IDs like `TML-2408`, ADR numbers, `PR #494`) are fine; only _transient_
IDs are banned.
