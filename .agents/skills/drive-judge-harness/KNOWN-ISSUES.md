...
4. **`tsc` confirms the breakage** (representative, with `skipLibCheck:false`):
   ...
   (`skipLibCheck:true` masks errors that live _inside_ the SDK's own `.d.ts`, but does **not** rescue a consumer that names an affected type directly — e.g. `import type { TurnEndedUpdate }` — because the member genuinely does not resolve.)
