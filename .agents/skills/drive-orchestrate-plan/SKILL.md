...
description: Use when the operator wants to drive a project plan to a review-clean state on the local branch via an iterate-implement-review loop.
metadata:
  version: "2026.4.29"
---
...
- The operator wants to drive the plan to a review-clean state on the local branch, before opening a PR.
...
- The orchestrator (you, the calling agent) has the operator's approval to delegate work to sub-agents.
...
- `drive-code-review` if the project lacks a `code-review.md` and you want one bootstrapped before iteration begins.
...
- **Intent fidelity:** a `SATISFIED` verdict is necessary but not sufficient. Cross-check against project intent before treating it as ground truth. The persona-doc's _human-in-the-loop probe_ fires here — but the plan-loop adds a concrete trigger (every reviewer verdict is intent-validated before triaging; see § Loop algorithm step 7).
- **Honest pushback acceptance:** when an implementer brings evidence that contradicts a reviewer finding, update the reviewer's record rather than rubber-stamping. This is the persona's _surface-conflicts-don't-merge-them_ applied to the implementer-vs-reviewer-finding case.
...
3. **Findings drift across rounds.** A fresh reviewer cannot remember the _reasoning_ behind their prior round's verdict — only what's on disk. Subtle calibrations ("F3's severity was deliberately `should-fix` because of X") get lost. The reviewer's mental model of the project rebuilds from scratch each round, which is expensive and error-prone.
...
- **A pivot of role intent** — rare. E.g. if the operator explicitly asks for a "fresh-eyes" reviewer pass, spawn fresh as a deliberate one-off and revert to the persistent reviewer for subsequent rounds. Document the deliberate choice in `code-review.md`.
...
The walkthrough is the operator's **primary review surface** for any single round. It must always reflect HEAD. A missing or stale walkthrough is a delegation-protocol failure on the same level as a missing verdict.
...
This is the most important way `drive-orchestrate-plan` differs from `drive-code-review`:

- `drive-code-review` is a **one-shot** branch review. Out-of-scope observations and "consider this for the future" notes are legitimate output — they go in a `Deferred` section, the reader can act on them or not.
...
Surface the inferred gate to the operator for confirmation, then write it back into the milestone definition in `plan.md` so subsequent rounds and milestones inherit it. Inferred-without-confirmation gates are a foot-gun: the operator often has tacit harness knowledge that doesn't surface until you propose a wrong gate.
...
4. **If implementer returns deferral requests**: surface to the operator as a structured decision (see § Escalation surface). Do not re-delegate until the operator has decided.
...
6. **Receive reviewer verdict.** One of `SATISFIED`, `ANOTHER ROUND NEEDED`, `ESCALATING TO OPERATOR`.
7. **Validate the review against intent.** Required step. The reviewer reasons forward from artifacts; you reason forward from intent (see § The three personas → Orchestrator → Epistemic frame). Read the verdict, the AC scoreboard delta, the new findings (and their severities), and any narrative artifacts the reviewer refreshed (`system-design-review.md`, `walkthrough.md`). Apply your project-level context to four questions:
   ...
   - **Did the reviewer let any architectural choice through that I should question?** The reviewer's role discipline is to validate against the spec; the orchestrator's role _is_ to second-guess design when intent demands it.
   ...
   This is a 2-minute skim, not a re-review of the diff. You are _applying_ the intent frame to the reviewer's output, not re-doing the reviewer's job. Possible actions, choose by judgment:
   ...
   - **Override a verdict** (rare): demote `SATISFIED` to `ANOTHER ROUND NEEDED` with rationale, recorded under an `## Orchestrator notes` section in `code-review.md`. Frequent overrides mean the reviewer is mis-calibrated — that's a separate problem worth raising with the operator.
...
8. **Triage verdict** (post intent-validation):
   - `SATISFIED` → report milestone completion to the operator, recommend the next milestone or transition to PR.
   - `ANOTHER ROUND NEEDED` → re-prompt implementer with the reviewer's findings; loop to step 2.
   - `ESCALATING TO OPERATOR` → surface the reviewer's concerns as a structured decision.
...
10. **Update durable docs and ADRs (conditional).** After a milestone reaches `SATISFIED`, update the consuming repo's durable documentation and ADRs to reflect any long-lived decisions or behavioural changes the milestone introduced. See § Updating docs and ADRs for the condition and mechanics. Do this as part of completing the milestone, not deferred to close-out.

## Updating docs and ADRs

Keeping durable documentation in step with the code is part of executing a milestone, not a separate phase. When a milestone lands a long-lived decision or a user-visible behavioural change, the documentation that records it should move in the same loop — while the context is fresh — rather than being reconstructed at close-out.

**Condition: only when the standard already exists in the repo.** Update docs/ADRs **only if the consuming repository already has an established documentation or ADR convention** — e.g. an `adrs/` directory, a `docs/` tree the team maintains, a `prisma/ignite` standard the repo defers to, or an equivalent. Do **not** invent a documentation standard, create an `adrs/` directory where none exists, or impose a structure the repo hasn't adopted. If the repo has no such convention, skip this step entirely (the decision still lives in the project `spec.md`, which close-out handles).

**What to update, when the condition holds:**

- **ADRs** for durable architectural decisions the milestone settles (a new convention, a system invariant, a technology choice). Follow the repo's existing ADR process and numbering.
- **Subsystem / module docs** for behavioural changes a future reader of that subsystem needs to know.
- Keep it proportional: a milestone that only moves runtime internals with no durable decision needs no doc change.

**How it relates to close-out.** The plan's final close-out task is the **final sweep**: it verifies nothing durable was missed, migrates anything still living only in the transient `projects/{project}/` artifacts, and then deletes the project folder. Updating docs/ADRs per-milestone here keeps that sweep small; it does not replace it.
...
- **Lead with the problem statement** before options, so the operator can form their own view before reading recommendations.
- **State a recommendation only when defensible** — e.g. you've checked the relevant code, evaluated the trade-off, and have evidence. If the trade-off is genuinely the operator's call, say so and do not recommend.
- **Translate the operator's response back into plan/spec edits before re-delegating.** A decision that lives only in the orchestrator's head is invisible to future sub-agent rounds. New scope → plan amendment. Renamed concept → spec amendment. Accepted deferral → record in `plan.md` § Open items.

## Replan protocol

The operator has explicitly opted into being involved on every replan, but the protocol is documented here so the orchestrator can surface replan triggers consistently.
...
- The operator adds scope mid-loop ("let's also rename X while we're here").

Triggered replan steps:

1. Orchestrator pauses the loop and surfaces the replan trigger to the operator as a structured decision (see § Escalation surface).
2. Operator decides scope: `accept-and-add-tasks` / `file-as-follow-up-ticket` / `accept-as-out-of-scope` / `restructure-the-plan`.
...
When the operator explicitly hands off the loop without availability for further input — phrases like _"continue without me to the end"_, _"use your best judgement"_, _"I'll be unavailable for comment"_, _"don't ask me, just drive"_ — the orchestrator runs in **unattended mode**. The defaults shift in two ways:

1. **Decisions that would normally be escalated to the operator are made in-place**, using the most defensible reading of project intent.
2. **Every such decision is logged**, in a format the operator can audit on return _without needing to cross-reference other documents_.

The operator has explicitly traded interactivity for a written audit trail; the trade is only honored if the trail is readable on its own.
...
- **Pre-existing flakes / unrelated failures.** If a non-milestone failure surfaces, log it. Fix only if it blocks a validation gate; otherwise leave for the operator.
...
- **No remote push beyond what the loop normally does.** Branch stays local; PR opening is a separate skill and is invoked only if the operator explicitly named it as part of the unattended scope.
- **No third-party automation expansions.** Don't authorize automated agents (e.g. a PR-comment bot's "shall I open a tracking issue?") to create artifacts in the team's trackers without operator approval. Decline politely; log the decline.
...
- The operator's prior decisions are mutually inconsistent in a way that requires their input to resolve.
...
**Location.** A gitignored scratch file under the repo's scratch convention (e.g. `wip/unattended-decisions.md`, or whatever local gitignored directory the repo uses for transient work). The file **must be gitignored** — it is the operator's personal review surface, not a shipped artifact. Confirm the location with the operator when entering unattended mode if the convention isn't already established.
...
- Append an entry every time you make a decision that would have been escalated under normal operation. This includes: triage decisions in place of the operator; declining out-of-scope findings; verdict overrides; accepted side-quests; declined offers from automated agents; deferring pre-existing failures.
...
The operator reading this log on return is **not in your context**: review artifacts may be deleted by close-out, finding IDs and round labels are meaningless without their source documents, and the operator has not been tracking the loop in real time. The format must absorb that asymmetry.

Concrete rules:

- **Do not refer to findings by ID** (`F17`, `F18`, `A02a`). Translate the substance into plain language: _"a doc comment in `cli.ts` cross-referenced the project's `spec.md`, which would become a dead link after close-out deleted the project folder"_.
...
- **Do not assume the reader can read other artifacts.** `code-review.md`, per-PR action JSON, and inflight delegation prompts are likely deleted, stale, or simply not where the operator is looking by the time they audit. Restate any context the entry depends on.
...
- **Make verification concrete.** Every entry includes a _How to verify_ section with a specific check the operator can run without further context: a file to read, a symbol to grep, a commit SHA to inspect, a behavior to observe.

The four questions an entry must answer (the operator has explicitly named these):
...
When the operator re-engages, hand them the log explicitly: list the decision titles inline, surface anything you flagged as needing their attention, and confirm whether any deferred items still need a destination (tracker ticket, plan amendment, follow-up PR). Do not assume they've read the log — the listing is the handoff.

## Project learnings

The orchestrator maintains a per-project `projects/{project}/learnings.md` recording patterns surfaced during this run — foot-guns, escapees, severity calibrations the operator weighed in on, classes-of-bug the spec didn't cover. Treat it as a working ledger, not a shipped artifact.
...
- **During the run**: the orchestrator appends a pattern entry whenever a non-trivial calibration is made (a foot-gun the operator pointed out, a severity recalibration that should propagate, a structural finding that turned into a plan amendment).
- **At close-out** (per `drive-close-project`): the orchestrator + operator review the learnings file together. Lessons that are durable cross-cutting knowledge migrate to the consuming repo's docs (e.g. an ADR, a team-level engineering doc, or a `prisma/ignite` doc — wherever the team keeps cross-cutting standards). Project-local lessons are dropped with the rest of the project folder.
...
- **Side-quests get explicit framing + their own commit.** Out-of-scope fixes (e.g. fixing a pre-existing flake the operator requests during the loop) commit separately with a scope-note in the commit message; the implementer should never bundle them with milestone work.
- **All three review artifacts produced or refreshed every round, no exceptions.** `code-review.md`, `system-design-review.md`, and `walkthrough.md` all update on every round (see § The artifact contract). The walkthrough is the operator's primary per-round review surface; missing it breaks their ability to review the round. If the reviewer returns without all three reflecting HEAD, re-prompt with a refresh-only delegation before accepting the verdict.
...
- **Operator decisions translate to plan/spec edits before re-delegating.** Any decision that affects future rounds belongs on disk, not in the orchestrator's working memory.
- **Track deferred items in `plan.md § Open items`**, not in conversation. The plan is the durable surface.
- **Update durable docs and ADRs as milestones land — but only where the repo already has that standard.** When a milestone settles a long-lived decision or user-visible behavioural change, update the consuming repo's existing ADR/doc conventions in the same loop (see § Updating docs and ADRs). Never invent a documentation standard the repo hasn't adopted; if no convention exists, skip it and let close-out handle the decision from `spec.md`.
...
| this skill          | `drive-code-review`              | `code-review.md` doesn't yet exist; scaffold via that skill's templates. |
...
See [`references/model-selection.md`](references/model-selection.md) for model guidance per agent role and the model + resume interaction.
