...
This is the single biggest way `drive-orchestrate-plan`'s reviewer differs from `drive-code-review`'s: every finding you file reaches the implementer in the next delegation. Findings that recommend "no action," "consider for the future," or "defer to another milestone" produce noise, dilute the implementer's attention, and obscure the real action items. Do not file them.

### The bar for filing a finding

The recommended action must be **addressable by the implementer in the current PR**, in either the current milestone or an explicitly named later milestone of _this_ PR. Apply this checklist before filing:
