...
- The operator wants to update _all_ skills, not just drive-*. This skill is scoped to `drive-*`. Use `npx skills update` (or the equivalent unscoped command) directly.
...
For each in-repo drive-* skill, record the canonical SKILL.md content _before_ update. This becomes the "old" side of cross-reference diffs in step 5.
...
Walk all non-drive in-repo skill files (`skills-contrib/**/SKILL.md` minus the drive-* paths, and the same for any other skill-root location). For each file, look for cross-references that _both_:
...
- **Disappeared** (section removed, file moved) → flag as a _broken_ reference. Propose a patch (delete the reference, or point at the closest surviving anchor with a `<!-- TODO: verify -->` comment).
...
- __Auto-install brand-new drive-_ skills the consumer never had._* It updates what is already installed. To install a new skill, run `npx skills add` directly.
...
2. **Patching project markdown.** Symptom: scope creeps to AGENTS.md, project specs, etc., and the patch set becomes unreviewable. Fix: scope is _non-drive in-repo skill files_ only; broader patching is its own job.
