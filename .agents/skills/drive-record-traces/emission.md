...
Validation happens **at emit time**. The emitter (§ Append protocol) validates the fully-merged event against the canonical arktype schema in [`schema.ts`](./schema.ts) _before_ writing, and refuses to append a non-conformant event (fail-closed: it exits non-zero and writes nothing). This is the first gate — a malformed line never reaches the file.
