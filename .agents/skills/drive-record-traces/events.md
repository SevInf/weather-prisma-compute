...
> **Note.** Tier-1 `CORRECT` requires all three components to be `"pass"`. Any `null` leaves the run `not computable` and the scorecard names the null component(s); any `"fail"` makes the run `INCORRECT`. This event is the _slot_ the judge fills; building the judge is a later slice.
