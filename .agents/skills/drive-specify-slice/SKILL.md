...
The slice spec is the **contract** that the slice plan + dispatch loop work against. It carries the decided design shape and the coherence rationale (slice-INVEST's _Small_ check, in prose). It is **post-discussion** — alternatives belong in the design-decisions record, not here.
...
Apply slice-INVEST (see [`docs/drive/principles/sizing.md`](../../docs/drive/principles/sizing.md)). The _Small_ check is: **manageable in a single code review**. One reviewer holds the slice's coherence in one sitting; the diff is rollback-able as one unit.
