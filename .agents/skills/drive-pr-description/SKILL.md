...
description: Use when the operator asks to create/open/submit/raise a PR, or requests reviewer-facing PR narrative (title, description, summary, branch overview, or squash-merge message). For any PR create or update flow, use this skill first to generate the title and description before running PR commands. Exclude requests focused on code critique or replying to review threads.
metadata:
  author: Prisma
  version: "2026.5.28"
---
...
- **Concise for quick reading**: Facilitate rapid comprehension by a reviewer
...
1. The title in its own markdown code block
2. The PR description body in a separate markdown code block
