...
A good manual-QA script is a structured accompaniment to a spec: it walks the runner (the `drive-qa-run` agent — or, in legacy contexts, a human) through the live system the way a user would drive it, and surfaces failures CI cannot catch (diagnostic clarity, end-to-end journey breaks, regressions of the original-bug-report flow, gate-of-gate sanity). It is **not** a checklist of `pnpm test` invocations dressed up in prose, and it is **not** the record of a particular QA pass — the latter is the _report_, produced by the runner skill against this script.

"Manual" describes _what is being run_ (the live system end-to-end, against real artefacts) and _what kind of inference_ the runner brings (judgement about diagnostic copy, file shape, journey feel — not just structural assertions). It does **not** mean "human-driven". The runner is autonomous and parallelises wherever the script's isolation tags say it can.
...
- You're _executing_ an existing manual-QA script and writing the run report. → `drive-qa-run`.
...
- **Script vs report.** This skill produces the _script_ — the durable test plan that lives alongside the spec and is re-run on every relevant change. The report (what was found, severity-classified, with reproduction artefacts) is produced by `drive-qa-run` per execution. Conflating the two leads to scripts that pre-classify findings before they're observed and scripts that get edited destructively when a QA round fills them in.
- **Manual QA's comparative advantage over CI is judgement, not throughput.** CI checks exit codes and structural shapes. The runner judges diagnostic clarity, output legibility, whether a generated file _looks right_, whether the developer journey _feels right_ — and it does so against an oracle the script names explicitly. Scenarios should call out what the runner is _looking at_, not just what it's _running_.
...
- **Exploratory charters catch what scripts can't.** Scripted scenarios catch _known unknowns_ — failure modes the author anticipated. **Exploratory** scenarios (charter + time-box) catch _unknown unknowns_ by inviting the runner to probe combinations the script didn't enumerate. Every non-trivial QA script should include at least one charter-style scenario.
...
- **Isolation tags are the parallelism contract.** Every scripted scenario carries a single isolation tag — `tmpdir`, `workspace`, `read-only`, or `external` — that tells the runner how to schedule it concurrently with others. The runner parallelises by default; isolation tags decide _where_ the scenario isolates, not whether it runs in series. Tag the strictest context the scenario actually needs; over-tagging defeats parallelism for no gain.
  - `tmpdir` — scenario only mutates state inside its own scratch directory (`$PN_QA_TMP/scenario-N`); reads from a shared read-only clone. Multiple `tmpdir` scenarios run concurrently with no extra setup.
  - `workspace` — scenario mutates the project's working tree (planted file in `skills/`, edited tracked file, populated `node_modules`, mutated DB at the workspace path). The runner allocates a `git worktree` per `workspace` scenario so they parallelise without races.
  - `read-only` — scenario reads but does not mutate any path the other scenarios touch. Trivially parallel.
  - `external` — scenario hits the network or another shared external resource (npm registry, GitHub API, real DB). Parallel-safe but rate-limit-aware; the runner caps `external` concurrency lower than the others.
- **Dependencies are explicit, not implicit.** If scenario B reads scenario A's output (a log line, a planted artefact, a populated cache), declare the dependency in B's Preconditions block by naming the prerequisite scenario by ID. Those declarations form the run's DAG. Scenarios that don't declare a dependency are independent — declaring "scenario N completed" out of caution kills parallelism for no gain.
...
| - | ------------------------- | ---------------------------- | --------- | ---------------- |
| 1 | <Short verb-phrase title> | <One-line user-facing claim> | tmpdir    | AC-1, AC-4       |
| 2 | <…>                       | <…>                          | workspace | AC-2             |
| N | Exploratory: <charter>    | Probe unanticipated states   | tmpdir    | (no AC; charter) |
...
>
> The **Isolation** column tells the runner how to schedule the scenario in parallel: `tmpdir` (own scratch dir, shared read-only clone), `workspace` (own `git worktree`), `read-only` (no isolation needed), or `external` (network-bound; rate-limit-aware). See Key Concepts for the full convention.
...
**Isolation:** `tmpdir` | `workspace` | `read-only` | `external` (pick one — see Key Concepts)
...
- <Prereq scenarios, if any — name them by ID, e.g. "scenario 2 completed". The runner treats these as DAG edges and waits for completion.>
...
- <Observable 1>
- <Observable 2 — call out what the human is _looking at_, not just exit codes>
...
The sign-off coverage map shows _which_ scenarios cover _which_ ACs. Result columns (pass/fail/N/A) are filled in by the runner's report, not by the script.

## Workflow

### 1. Read the spec, the plan, and the PR diff
...
### 2. Inventory candidate scenarios against the litmus test
...
It is _correct and expected_ for some ACs to come out N/A. If you find you've labelled every AC with a scenario, re-check — you've probably padded the script with "run our tests" rituals.

### 3. Draft the TOC first
...
2. **Covers** — AC IDs.
3. **Isolation** — exactly one of `tmpdir` / `workspace` / `read-only` / `external`. Determines how the runner schedules the scenario in parallel with others. Pick the strictest tag the scenario actually requires:
   - `tmpdir` if the scenario only mutates `$PN_QA_TMP/scenario-N` and reads from a shared read-only clone.
   - `workspace` if the scenario mutates anything inside the project's working tree (planted file, edited tracked file, populated `node_modules`, etc.). The runner allocates a `git worktree` per `workspace` scenario.
   - `read-only` if the scenario reads but does not mutate any path another scenario touches.
   - `external` if the scenario hits the network (npm registry, GitHub API) or any shared external resource. The runner caps `external` concurrency lower than the others.
4. **Oracle** — the source-of-truth comparison standard. Especially load-bearing on judgement scenarios.
5. **Preconditions** — environment + state + prereq scenarios. Bulleted, scannable. **Prereq scenarios become DAG edges** — name them by ID (e.g. "scenario 2 completed"); the runner holds the dependent scenario back until they finish.
6. **Steps** — numbered, copy-pasteable, real. No sketches like "run the CLI command" — paste the actual command.
7. **What you should see** — observables, including what the runner is _looking at_. Call out diagnostic copy, file shape, output legibility — anything tests can't easily assert.
8. **Failure modes** — explicit failure categories that constitute a finding. **Do not pre-classify severity** — severity is a property of _what was found_ in a runtime context, and it belongs in the runner's report.
9. **Restore** (if and only if the scenario mutates state outside its isolation context, or the script-author wants explicit evidence the scenario's claimed cleanup actually fires) — restoration steps + a `git status` check.

For exploratory scenarios, the shape is different: charter, covers, isolation, time budget, notes-capture instruction. No steps, no failure modes (the runner discovers them). Charters that probe `init`-style flows usually tag `tmpdir`; charters that probe in-tree refactors tag `workspace`.

### 5. Add the "Scenarios deliberately not in this script" table

This is load-bearing. It documents which ACs you chose _not_ to cover and why. A future reader (or a different QA round) will check it before assuming a coverage gap means an oversight.
...
1. **Conflating script with report.** Symptom: severity tags ("blocker", "high") on failure modes in the script; a "Result" column in the sign-off matrix; the doc gets edited destructively when a QA round fills it in. Fix: severity belongs in the runner's report; the script enumerates failure-mode _categories_, not severities. Results never live in the script.
...
5. **No observation hooks.** Symptom: scenario lists commands but its "What you should see" is "no errors". Fix: name the specific things the human is looking _at_ — the diagnostic copy, the file shape, the output legibility. The oracle tells the runner what to compare; "what you should see" tells them where to look.
...
14. **Over-tagging isolation.** Symptom: every scenario tagged `workspace` "to be safe". Fix: the runner allocates a fresh `git worktree` per `workspace` scenario; tagging a tmpdir-only scenario as `workspace` doubles its setup cost for no benefit. Pick the strictest tag the scenario _actually_ needs.
...
16. **Cross-scenario state assumptions.** Symptom: scenario 4's Preconditions assume scenario 3 left a populated `node_modules` even though scenario 3 has Isolation `tmpdir`. Cause: the author conflated "ran earlier" with "left state behind". Fix: the runner gives each scenario a clean isolation context. If scenario 4 needs the artefact, declare the dependency _and_ have scenario 3 either copy the artefact into a shared location or be tagged `workspace` so the worktree persists into scenario 4's context.
...
- [ ] Every scenario's **Isolation** field is exactly one of `tmpdir` / `workspace` / `read-only` / `external`, picking the strictest tag the scenario actually requires.
- [ ] Cross-scenario dependencies appear in the dependent scenario's Preconditions list with the prerequisite scenario named by ID; no phantom dependencies.
