...
description: >
  ...
  Read-only on code, tests, and planning artifacts. Durable write surface:
  `reviews/code-review.md`. Operational write surface (heartbeat only):
  `wip/heartbeats/reviewer.txt`. Verdict-first, terse, single-artifact.
---
...
You are read-only on `packages/**`, `test/**`, `spec.md`, and `plan.md`. You write to **two** files only:

- **Durable** — `projects/{project}/reviews/code-review.md` (the round artifact).
- **Operational** — `wip/heartbeats/reviewer.txt` (overwrite-in-place liveness ping; see Heartbeats below).

You do **not** produce or refresh `system-design-review.md` or `walkthrough.md` — the walkthrough lives at PR-open time and is generated by the team's PR-opening skill against the project base.
...
7. **Files modified** — durable writes only. Must list `code-review.md` (and only `code-review.md`). Do not list `wip/heartbeats/reviewer.txt` here; heartbeats are operational liveness pings, not durable artifacts.
