...
the _loop_ — DoR / WIP / DoD / reviewer / intent-validation / escalation /
slice-DoD-close.
...
- **Re-prompt the reviewer** with a focused gap (_"you said SATISFIED but missed X; revisit"_). Preserves the reviewer's role.
...
6. **Operator-set custom stop.** Declared in `drive/plan/README.md` (e.g. _"any dispatch that touches `packages/3-extensions/migration/*` halts for review before merge"_).
...
3. **Findings drift across rounds.** A fresh reviewer can't remember the _reasoning_ behind their prior verdict — only what's on disk. Subtle calibrations get lost.
...
When the user explicitly hands off — _"continue without me to the end"_, _"use your best judgement"_, _"I'll be unavailable"_ — defaults shift:
